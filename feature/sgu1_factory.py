...
#         self.close = self._minute_df['close']
#         self.vol = self._minute_df['vol']
#         self.amount = self._minute_df['amount']
...
#         # lagged labels
#         raw_rr = ts_sub(ts_log(self._minute_df['high']), ts_log(self._minute_df['low']))
#         for L in range(1, 6):
#             features[f'f_lag_label_{L}'] = ts_delay(raw_rr, L)

#         return pd.DataFrame(features, index=self._minute_df.index)
    
#     def _get_normalized_time(self):
#         """Convert YYYYMMDDHHMMSS to minutes since market open"""
#         time_series = pd.to_datetime(self._minute_df['trade_time'], format='%Y%m%d%H%M%S')
