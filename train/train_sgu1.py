...
from models.dataloader import SGU1DataPro
...
def pro_dataloader(start:int=20240501, end:int=20240531):
    ...
    for date_str in tqdm(sample_dates, desc=f"Loading SGU1 Data ({start}-{end})"):
        ...
        day_data = loader.gen_dataset(k_future=10, event_step=8)
...
def pro_sgu1_pipeline(s1=(20240501, 20240518), s2=(20240519, 20240525)):
    s1_data = pro_dataloader(start=s1[0], end=s1[1])
    s2_data = pro_dataloader(start=s2[0], end=s2[1])
    ...
    # save models
    os.makedirs('checkpoints/sample_xgb_pro', exist_ok=True)
    save_path = f"checkpoints/sample_xgb_pro/sgu1_xgboost_{s1[0]}_{s1[1]}.json"
    model.save(save_path)
    print(f"\nModel saved to {save_path}")
    img_dir = 'D:/UW/Course/2026 WINTER/522_trade_sys/replication/img/sgu1_graph'
    os.makedirs(img_dir, exist_ok=True)
    evaluate_sgu1_comparison(model, X_val, y_val, os.path.join(img_dir, 'sgu1_val_pro.png'))
    plot_importance(model, os.path.join(img_dir, 'sgu1_importance_pro.png'))
...
if __name__ == '__main__':
    ...
    sgu1, val_corr = pro_sgu1_pipeline()