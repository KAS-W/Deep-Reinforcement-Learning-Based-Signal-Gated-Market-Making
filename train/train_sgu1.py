...
def pro_dataloader(start:int=20240501, end:int=20240531, event_step=19):
    ...
    for date_str in tqdm(sample_dates, desc=f"Loading SGU1 Data ({start}-{end})"):
        ...
        snap_df = snap_df[snap_df['trade_time'] >= 93500000].copy()
        tick_df = tick_df[tick_df['trade_time'] >= 93500000].copy()

        loader = SGU1DataPro(tick_df=tick_df, snap_df=snap_df)

        day_data = loader.gen_dataset(k_future=10, event_step=event_step)
